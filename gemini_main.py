...
def set_environment(seed, llm_name='gemini'):
    ...
    return seed, model, device

def load_data(nrows=1500):
    # csv file paths
    listening_log_path = 'gs://skip-prediction-bucket//used_testset_files//'
    track_features_path = 'gs://skip-prediction-bucket//track_features//'
...
def initialise_model(model='gemini'):
...
def get_model_response(prompt, model, max_attempts=3):
    valid_responses = {'True', 'False'}
    attempts = 0

    # Loop until a valid response is returned
    while attempts < max_attempts:
        # Send the prompt to the gemini model for a response
        response = model.chat(prompt).text.strip()
...
def exp_one_oneshot_prompt(current_tracks, num_tracks, track_id, track_features_dict, oneshot_sample_set, seed):
    ...
    one_shot_examples, one_shot_current_tracks, one_shot_ground_truths = generate_oneshot_examples(oneshot_sample_set, num_tracks, seed, track_features_dict, experiment='exp1')
...
def exp_two_oneshot_prompt(current_tracks, num_tracks, track_id, track_features_dict, oneshot_sample_set, seed):
    ...
    # Generate one_shot examples and ground truths
    one_shot_examples, one_shot_current_tracks, one_shot_ground_truths = generate_oneshot_examples(oneshot_sample_set, num_tracks, seed, track_features_dict, experiment='exp2')
...
def exp_three_oneshot_prompt(current_tracks, num_tracks, track_id, track_features_dict, oneshot_sample_set, seed):
    ...
    # Generate one_shot examples and ground truths
    one_shot_examples, one_shot_current_tracks, one_shot_ground_truths = generate_oneshot_examples(oneshot_sample_set, num_tracks, seed, track_features_dict, experiment='exp3')
...
def exp_four_oneshot_prompt(current_tracks, num_tracks, track_id, session_context, track_features_dict, oneshot_sample_set, seed):
    ...
    # Generate one_shot examples and ground truths
    one_shot_examples, one_shot_current_tracks, one_shot_ground_truths = generate_oneshot_examples(oneshot_sample_set, num_tracks, seed, track_features_dict, experiment='exp4')
...
def exp_five_oneshot_prompt(current_tracks, num_tracks, track_id, session_context, track_features_dict, oneshot_sample_set, seed):
    ...
    # Generate one_shot examples and ground truths
    one_shot_examples, one_shot_current_tracks, one_shot_ground_truths = generate_oneshot_examples(oneshot_sample_set, num_tracks, seed, track_features_dict, experiment='exp5')
...
def generate_oneshot_examples(sample_set, num_tracks_in_prompt, seed, track_features_dict, num_examples=1, experiment='exp1'):
    one_shot_examples = []
    ...
    # Set random seed for reproducibility of sessions selected for one-shot example set
    random.seed(seed)
...
# Function for getting track details appropriate for the given experiment
...
def run_experiment(prompts, model, experiment='exp1', shots='zero_shot'):
...
def save_results(experiment, model, seed, results_df):
    base_path = f'gs://skip-prediction-bucket//{model}//Seed_{seed}//{experiment}//'
...
def initialize_data(track_features_df, testset_separate, listen_log_df):
    ...
    return track_features_dict, oneshot_sample_set, listen_log_df, dummy_train_data, majority_labels
...
def run_and_save_experiments(seed, prompts, model, llm, listen_log_df, majority_labels):
    results = {}

    for key, prompt_set in prompts.items():
        exp, shot = key.split('_', 1)
        results[key] = run_experiment(prompt_set, llm, exp, shot)
...
def main():
    ...
    # Set environment
    seed, model, device = set_environment(args.seed, args.model_name)
    ...
    run_and_save_experiments(seed, prompts, model, llm, listen_log_df, majority_labels)
